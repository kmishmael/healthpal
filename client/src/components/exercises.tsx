...
const retrieveExerciseList = async (userId: any) => {
  const response = await axios.get(API_URL + `exercise/${userId}`);
  return response.data.data;
};

const retrieveWorkouts = async (userId: any) => {
  const response = await axios.get(API_URL + `exercises/${userId}`);
  console.log(response.data.data["this_month"]);
  return response.data.data["this_month"];
};

const addExercise = async (userId: any, data: any, refetch: any) => {
...
const updateExercise = async (userId: any, data: any, refetch: any) => {
  const response = await axios
    .put(API_URL + `exercise/${data.exercise_id}`, {
      user_id: userId,
      ...data,
    })
    .finally(() => {
...
const updateWorkout = async (userId: any, data: any, refetch: any) => {
  const response = await axios
    .put(API_URL + `exercises/${userId}/${data.exercises_id}`, {
      user_id: userId,
      ...data,
    })
    .finally(() => {
...
const addWorkout = async (userId: any, data: any, refetch: any) => {
  const response = await axios
    .post(API_URL + `exercises/${userId}`, {
      user_id: userId,
      ...data,
    })
    .finally(() => {
...
const deleteExercise = async (foodId: any, refetch: any) => {
  const response = await axios
    .delete(API_URL + `exercise/${foodId}`)
    .finally(() => {
      refetch();
    });
  return response.data;
...
const deleteWorkout = async (mealId: any, userId: any, refetch: any) => {
  const response = await axios
    .delete(API_URL + `exercises/${userId}/${mealId}`)
    .finally(() => {
      refetch();
    });
  return response.data;
...
export default function Meals() {
  ...
  const {
    ...
    queryKey: ["mealsData"],
    queryFn: () => retrieveWorkouts(token?.user.id),
  ...
  const {
    ...
    queryKey: ["foodData"],
    queryFn: () => retrieveExerciseList(token?.user.id),
    refetchOnWindowFocus: false,
  });
                  ...
                  <div className="w-full flex justify-center items-center text-center">
                    ...
                    >
                      <div className="flex gap-1 items-center">
                        <IoAdd className="w-5 h-5 font-bold" /> Workout
                      </div>
                    </button>
                  </div>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="w-full flex justify-center items-center text-center">
                    ...
                    >
                      <div className="flex gap-1 items-center">
                        <IoAdd className="w-5 h-5 font-bold" /> Exercise
                      </div>
                    </button>
                  </div>
                </CardContent>
                        ...
                        customFoods.map((c: any) => (
                          <TableRow>
                            ...
                            <TableCell className="flex gap-3">
                              ...
                              <Button
                                onClick={() =>
                                  deleteExercise(c.id, foodRefetch)
                                }
                                className="text-xs px-3 w-14 py-1.5 border rounded"
                              >
                                Delete
...
function AddCustomMeal({
  ...
  function handleForm(data: any) {
    try {
      //setIsOpen(false);
      let amount = parseInt(data.time);
      if (isNaN(amount)) {
        amount = 10;
      }
      const d = {
        type: type,
        exercise_id: food.value,
        time: amount,
        user_id: token?.user.id,
      ...
      console.log(d)
      addWorkout(token?.user.id, d, refetch);
      setIsOpen(false);
    } catch {}
  }
  return (
                    ...
                    <CardContent>
                      <form onSubmit={handleSubmit(handleForm)}>
                        <div className="grid w-full items-center gap-4">
                          ...
                          <div className="flex flex-col gap-2 space-y-1.5">
                            <Label htmlFor="name">Type of Workout</Label>
...
function AddCustomExercise({
  ...
  function handleForm(data: any) {
    try {
      ...
      let d: any = {};
      let sets: number | null = parseInt(data.sets);
      if (!isNaN(sets)) {
        d["sets"] = sets;
      }
      let reps: number | null = parseInt(data.reps);
      if (!isNaN(reps)) {
        d["reps"] = reps;
      }
      d = {
      ...
      console.log(d);
      addExercise(token?.user.id, d, refetch);
      setIsOpen(false);
    } catch {}
  }
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <Card className="w-full">
                    <CardHeader>
                      ...
                      <CardDescription>
                        Add a custom Exercise to your workouts
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <form onSubmit={handleSubmit(handleForm)}>
...
function EditCustomFood({
  ...
  function handleForm(data: any) {
    try {
      ...
      updateExercise(data.user_id, d, refetch);
      setIsOpen(false);
    } catch {}
  }
...
function EditCustomMeal({
  ...
  const [food, setFood] = useState<any>(
    inputData == null
      ...
      : {
          ...
          label: options.filter((c: any) => c.value == inputData?.food_id)[0]
            .label,
        }
  );
  ...
  function handleForm(data: any) {
    try {
      ...
      updateWorkout(token?.user.id, d, refetch);
      setIsOpen(false);
    } catch {}
  }
  return (
